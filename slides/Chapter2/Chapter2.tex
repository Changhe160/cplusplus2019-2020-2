...
\begin{columns}[t]
\column{0.8\textwidth}
\begin{yellowblock}{标识符命名小建议}
		\begin{itemize}
			\item 对象名一般小写，如name,而不是NAME；
            \item 应使用能帮助记忆的名字，如salary，而不是s；
            \item 有多个单词组成时，单词之间可用下划线或是内嵌单词的第一个字母大写；\\如student\_name或studentName。
		\end{itemize}
	\end{yellowblock}
\end{columns}
\end{frame}
...
\begin{greenblock}<2->{思考：一个\texttt{char}类型表示范围？}	
\onslide<3->{一个 \texttt{ char}类型表示范围为[1000 0000, 0111 1111]即-128至127。}
\end{greenblock}
...
\begin{block}{实型常量的表示方法}
		\begin{itemize}
			\item 小数形式：如~3.14159~~~~~4.235
            \item 指数形式：如~3.14159E0~~~~4e6 \\~~
		\end{itemize}
\end{block}
\column{0.45\textwidth}
...
\begin{block}{字符和字符串常量}		
		\begin{itemize}
			\item 字符常量：由单引号括起来单个字符，如：~~\texttt{\alert{'a'}}~~~~\texttt{'1'}~~
            \item 字符串常量：由双引号括起来的字符，如：~~\texttt{\alert{"a"}}~~~~\texttt{"Mandy"}。自动在末尾添加空字符（\texttt{'\textbackslash 0'}）
            \item 特殊字符：用\alert{转义序列}表示方法，如：\textbackslash n~换行符、\textbackslash r~回车符，\textbackslash\textbackslash~反斜线等\\
...
\begin{block}{前缀和后缀}
		 \begin{itemize}
			\item 为整型、实型和字符型常量添加前缀或后缀，可以改变其默认类型
            \item 例如：\\
            \begin{tabular}{ll}
          % after \\: \hline or \cline{col1-col2} \cline{col3-col4} ...
...
\begin{block}{左值和右值}
	\begin{itemize}
        \item 左值所在的内存空间的地址是可以获取的（使用取址符 \texttt{\&}），但右值的地址是无法得到的
...
\begin{block}{优先级和结合性}
	\begin{itemize}
        \item \alert{优先级}和\alert{结合性}共同决定了运算中的优先关系
...
\begin{block}<2->{}
\begin{spacing}{1}
优先级：\alert{加、减}~<~\alert{乘、除、求余}~<~\alert{单目}\\
结果：\alert{右值}
\end{spacing}
\end{block}
...
\pause
    \item 两侧的操作对象的类型不相同，会进行类型转换，但右侧本身的不会发生变化，如
\pause
\begin{lstlisting}
int i = 0;
double d = 3.14159;
...
\begin{spacing}{1.}
优先级：\\
赋值运算符~\texttt{<}~~\textcolor{blue}{逻辑与\texttt{\&\&}~、逻辑或\texttt{||}}~\texttt{<}~\textcolor{red}{关系运算符}~\texttt{<}~算术运算符~\texttt{<}~\textcolor{blue}{逻辑非\texttt{!}}
\end{spacing}
\end{block}
\pause
\begin{block}{ }
...
\begin{figure}[!h]
\centering
\includegraphics[width=\textwidth]{luoji.png}
\end{figure}
\end{yellowblock}
\end{columns}
\end{frame}
...
\begin{tabular}{ll}
  % after \\: \hline or \cline{col1-col2} \cline{col3-col4} ...
  (1)\texttt{a+b>c+d} & (2)\texttt{a=(b=4)+(c=6)} \\
  (3)\texttt{a+=c=b*a} & (4)\texttt{d=(a++)-(++b)+c--} \\
  (5)\texttt{--a||b<d\&\&c--} & ~ \\
\end{tabular}
...
\begin{figure}[!h]
\centering
\includegraphics[width=\textwidth]{tiaojian.png}
\end{figure}
\end{yellowblock}
\pause
\column{0.5\textwidth}
...
\begin{frame}[fragile]{2.5 表达式\normalsize{~---~位运算符}}
\begin{itemize}
  \item 位运算符：~\verb;~;~、\verb;<<;~、\verb;>>;~、\texttt{\&}~、\texttt{|} ~、~\verb;^;（优先级由高到低）
  \item 运算的对象是\alert{整型对象}，处理二进制数
\end{itemize}
\pause
\begin{columns}[t]
\column{0.95\textwidth}
\begin{blueblock}{~~\texttt{short a = 3, b = 5;}}
\begin{center}
...
\begin{greenblock}{问题}
是先计算\texttt{i++}还是\texttt{i*2}，如果先计算表达式 \texttt{i*2}再计算表达式\texttt{i++}，那么结果为\texttt{0}；反过来，结果则为\texttt{2?}
\end{greenblock}
...
\begin{itemize}
  \item \texttt{VS}编译器，\texttt{j}的值为\texttt{0}
  \item \texttt{GCC}编译器，\texttt{j}的值为\texttt{2}
\end{itemize}
\end{greenblock}
\begin{yellowblock}<3->{建议}
...
\begin{block}{隐式转换}
	\begin{itemize}
		\item 比int类型小的整型类型提升为较大的整型类型
		\item 将表达式的值转换为布尔值
        \item 初始化过程中，初始值转换成定义对象的类型
        \item 算术表达式中，运算结果转换为运算对象中最宽（大）的数据类型
	\end{itemize}
\end{block}
\end{columns}
\end{frame}
...
\begin{figure}[!h]
\centering
\includegraphics[width=\textwidth]{leixingzhuanhuan.png}
\end{figure}
\end{yellowblock}
\end{columns}
\end{frame}
