    ...
    statement2;  //分支语句2
     ...
     \end{lstlisting}
		...
		\begin{column}{0.48\textwidth}
			\begin{figure}[ht]
				\centering
				\includegraphics[scale=0.38]{IF_statement.png}
...
\begin{frame}[fragile]
	...
	\begin{thinkblock}{思考：}
		...
		\onslide<2->{因为\texttt{else}可以省略，而\texttt{if}不能省略}
	\end{thinkblock}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{block}{\texttt{switch}语句语法规则：}
		\begin{itemize}
			\item 每个开关入口可以\alert{对应多个标签值}，执行相同的操作\\
			      \begin{lstlisting}[basicstyle=\small\ttfamily]
        /*...*/
        switch(score/10){
        case 9:case 10:
...
\begin{frame}[fragile]
	...
	\begin{block}{\texttt{switch}语句语法规则：}
        ...
        case 7:
            ...
            break;
            \end{lstlisting}
            编译正确，但当进入开关case 7时，由于对象c未定义，出现错误
		\end{itemize}
	\end{block}
\end{frame}
...
\end{frame}
\begin{frame}[fragile]
	\frametitle{3.2 循环结构\small{—\texttt{while}语句}}
	% \framesubtitle{——\texttt{while}语句}
...
do{
    statement; //循环体语句
		...
		\begin{column}{0.4\textwidth}
			\begin{figure}[ht]
				\centering
				\includegraphics[scale=0.4]{dowhile_statement.png}
...
\begin{frame}[fragile]
	\frametitle{3.2 循环结构\small{—\texttt{do while}语句}}
	% \framesubtitle{——\texttt{do while}语句}
	\begin{columns}
		\begin{column}{0.5\textwidth}
...
for(int i = 1; i <= 100; ++i){
    sum += i;
		...
		\begin{column}{0.48\textwidth}
			\begin{figure}[ht]
				\centering
				\includegraphics[scale=0.6]{Fig3-1.jpg}
...
\begin{frame}[fragile]
	...
	\begin{abstractblock}{跳转语句用于中断当前的执行顺序，包括：}
		\begin{itemize}
			\item \textcolor[rgb]{0,0,1}{\texttt{break}}语句
			\item \textcolor[rgb]{0,0,1}{\texttt{return}}语句：
			      \begin{lstlisting}[basicstyle=\small\ttfamily]
    int main(){
        return 0; /*返回一个整型值*/
    }
\end{lstlisting}\ttfamily
			\item \textcolor[rgb]{0,0,1}{\texttt{continue}}语句
			\item \texttt{goto}语句（不作介绍）
		\end{itemize}
	\end{abstractblock}
\end{frame}
...
\vspace{-5mm}
			\begin{column}{0.94\textwidth}
				\begin{lstlisting}[numbers=left,numberstyle=\small,firstnumber=18,basicstyle=\small\ttfamily]
            case -1: case 2:
                cout << "你赢了!" << endl;
            }
